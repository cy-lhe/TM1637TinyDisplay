...
#include <TM1637TinyDisplayViaPCA9533.h>
#include <Arduino.h>
...
TM1637TinyDisplayViaPCA9533::TM1637TinyDisplayViaPCA9533(Pca9533::PCA9533* extPorts, Pca9533::pca9533_pin_t pinClk, Pca9533::pca9533_pin_t pinDIO, unsigned int bitDelay, unsigned int scrollDelay, bool flip) {
  m_extPorts = extPorts;
  // Pin settings
  m_pinClk = pinClk;
  m_pinDIO = pinDIO;
...
void TM1637TinyDisplayViaPCA9533::begin() {
  // Set the pin direction and default value.
  // Both pins are set as inputs, allowing the pull-up resistors to pull them up
  m_extPorts->setMode(m_pinClk, Pca9533::PIN_HIGH);
  m_extPorts->setMode(m_pinDIO, Pca9533::PIN_HIGH);
...
void TM1637TinyDisplayViaPCA9533::flipDisplay(bool flip) {
  m_flipDisplay = flip;
  writeBuffer();
}

bool TM1637TinyDisplayViaPCA9533::isflipDisplay() {
  return(m_flipDisplay);
}

void TM1637TinyDisplayViaPCA9533::setBrightness(uint8_t brightness, bool on) {
...
void TM1637TinyDisplayViaPCA9533::setScrolldelay(unsigned int scrollDelay) {
  m_scrollDelay = scrollDelay;
}

void TM1637TinyDisplayViaPCA9533::writeBuffer() {
  uint8_t dot = 0;
...
void TM1637TinyDisplayViaPCA9533::readBuffer(uint8_t* buffercopy) {
  for (uint8_t k = 0; k < MAXDIGITS; k++) {
    buffercopy[k] = digitsbuf[k];
  }
}

void TM1637TinyDisplayViaPCA9533::setSegments(const uint8_t segments[], uint8_t length, uint8_t pos) {
...
void TM1637TinyDisplayViaPCA9533::setSegments(uint8_t A, uint8_t pos) {
  digitsbuf[pos] = A;
  writeBuffer();
}

void TM1637TinyDisplayViaPCA9533::clear() {
  // digits[MAXDIGITS] output array to render
  memset(digits, 0, sizeof(digits));
  setSegments(digits);
}

void TM1637TinyDisplayViaPCA9533::showNumber(int num, bool leading_zero, uint8_t length, uint8_t pos) {
  showNumber(long(num), leading_zero, length, pos);
}

void TM1637TinyDisplayViaPCA9533::showNumber(long num, bool leading_zero, uint8_t length, uint8_t pos) {
  if (leading_zero) {
...
void TM1637TinyDisplayViaPCA9533::showNumber(double num, uint8_t decimal_length, uint8_t length, uint8_t pos) {
  int num_len = 0;
...
void TM1637TinyDisplayViaPCA9533::showNumberDec(int num, uint8_t dots, bool leading_zero,
  uint8_t length, uint8_t pos) {
  showNumberBaseEx(num < 0 ? -10 : 10, num < 0 ? -num : num, dots, leading_zero, length, pos);
}

void TM1637TinyDisplayViaPCA9533::showNumberHex(uint16_t num, uint8_t dots, bool leading_zero,
  uint8_t length, uint8_t pos) {
  showNumberBaseEx(16, num, dots, leading_zero, length, pos);
}

void TM1637TinyDisplayViaPCA9533::showNumberBaseEx(int8_t base, uint16_t num, uint8_t dots, bool leading_zero,
  uint8_t length, uint8_t pos) {
  bool negative = false;
  if (base < 0) {
    base = -base;
...
void TM1637TinyDisplayViaPCA9533::showString(const char s[], uint8_t length, uint8_t pos, uint8_t dots) {
  // digits[MAXDIGITS] output array to render
  memset(digits, 0, sizeof(digits));
...
void TM1637TinyDisplayViaPCA9533::showString_P(const char s[], uint8_t length, uint8_t pos, uint8_t dots) {
  // digits[MAXDIGITS] output array to render
  memset(digits, 0, sizeof(digits));
...
void TM1637TinyDisplayViaPCA9533::showLevel(unsigned int level, bool horizontal) {
  // digits[MAXDIGITS] output array to render
  memset(digits, 0, sizeof(digits));
...
bool TM1637TinyDisplayViaPCA9533::Animate(bool loop) {
...
void TM1637TinyDisplayViaPCA9533::startAnimation_P(const uint8_t(*data)[MAXDIGITS], unsigned int frames, unsigned int ms) {
  startAnimation(data, frames, ms, true);
}

void TM1637TinyDisplayViaPCA9533::startAnimation(const uint8_t(*data)[MAXDIGITS], unsigned int frames, unsigned int ms, bool usePROGMEM) {
  if (usePROGMEM) {
    m_animation_type = 2;
  } else {
    m_animation_type = 1;
...
void TM1637TinyDisplayViaPCA9533::stopAnimation() {
  m_animation_type = 0;
}

void TM1637TinyDisplayViaPCA9533::startStringScroll_P(const char s[], unsigned int ms) {
  startStringScroll(s, ms, true);
}

void TM1637TinyDisplayViaPCA9533::startStringScroll(const char s[], unsigned int ms, bool usePROGMEM) {
  if (usePROGMEM) {
...
void TM1637TinyDisplayViaPCA9533::showAnimation(const uint8_t data[][4], unsigned int frames, unsigned int ms) {
  // Animation sequence
...
void TM1637TinyDisplayViaPCA9533::showAnimation_P(const uint8_t data[][4], unsigned int frames, unsigned int ms) {
...
void TM1637TinyDisplayViaPCA9533::bitDelay() {
  delayMicroseconds(m_bitDelay);
}

void TM1637TinyDisplayViaPCA9533::start() {
  // pinMode(m_pinDIO, OUTPUT);
    m_extPorts->setMode(m_pinDIO, Pca9533::PIN_LOW);

  bitDelay();
}

void TM1637TinyDisplayViaPCA9533::stop() {
  // pinMode(m_pinDIO, OUTPUT);
    m_extPorts->setMode(m_pinDIO, Pca9533::PIN_LOW);
...
bool TM1637TinyDisplayViaPCA9533::writeByte(uint8_t b) {
  uint8_t data = b;
  ...
  for (uint8_t i = 0; i < 8; i++) {
    ...
    m_extPorts->setMode(m_pinClk, Pca9533::PIN_LOW);
  ...
  uint8_t ack = m_extPorts->digitalRead(m_pinDIO);
  if (ack == 0)
    //    pinMode(m_pinDIO, OUTPUT);
    m_extPorts->setMode(m_pinDIO, Pca9533::PIN_LOW);
...
void TM1637TinyDisplayViaPCA9533::showDots(uint8_t dots, uint8_t* digits) {
...
uint8_t TM1637TinyDisplayViaPCA9533::encodeDigit(uint8_t digit) {
  // return digitToSegment[digit & 0x0f] using PROGMEM
  return pgm_read_byte(digitToSegment + (digit & 0x0f));
}

uint8_t TM1637TinyDisplayViaPCA9533::encodeASCII(uint8_t chr) {
